...
from crc import file_crc
from codes import RequestCode

class Client:
    HEADER_SIZE = 23
    PACKET_SIZE = 1024
    PAYLOAD_HEADER_SIZE = 267
    ...
    def get_requests(self):
        while self.running: # keep receiving requests until the client disconnects or an error occurs
            try:
                header = self.connection.recv(Client.HEADER_SIZE) # 16 bytes for client_id, 1 byte for version, 2 bytes for code, 4 bytes for payload size
                if not header:
                    print(f"Error: failed to receive data from {self.address}.")
                    self.close_connection()
                    return
                if len(header) != Client.HEADER_SIZE:
                    raise ValueError()
            ...
            match code:
                ...
                case _: # default case, handle unknown request codes
                    print(f"Error: received unknown request code ({code}) from {self.address}.")
    ...
    def handle_register(self, payload):
        ...
        if client_exists is None:
            ...
            return
        if client_exists:
            print(f"{self.address} tried to register with the name {payload.decode().strip('\x00')}, but it's already taken.")
            ...
            if not self.database.register_client(self.client_id, payload.decode()):
                ...
                self.close_connection()
                return
            print(f"{self.address} registered with the name {payload.decode().strip('\x00')}.")
            self.name = payload.decode().strip('\x00')
    ...
    def handle_login(self, payload):
        ...
        if client_exists is None:
            ...
            return
        if client_exists:
            print(f"{self.address} logged in with the name {payload.decode().strip('\x00')}")
            self.name = payload.decode().strip('\x00')
            ...
            print(f"{self.address} tried to log in with the name {payload.decode().strip('\x00')}, but it's not registered " +
    ...
    def handle_save_file(self, payload):
        try:
            ...
            content_size, decrypted_size, packet_number, total_packets, file_name = struct.unpack("<IIHH255s", payload[:Client.PAYLOAD_HEADER_SIZE])
            ...
            encrypted_data = payload[Client.PAYLOAD_HEADER_SIZE:Client.PAYLOAD_HEADER_SIZE + Client.PACKET_SIZE] # get the encrypted data from the payload
    ...
    def close_connection(self):
        try:
            ...
            print(f"Error: failed to close connection with {self.address}.")
